...
class LMNN(DML_Algorithm, ClassifierMixin):
    ...
    def __init__(self,
        ...
        self.mu = mu
        self.k = k
        self.soft_comp_interval = soft_comp_interval
    ...
    def _SDP_fit(self, X, y):
        ...
        self.initial_error_ = self._compute_error(self.mu, M, X, y, target_neighbors, impostors)
        err_prev = err = self.initial_error_

        stop = False

        while not stop:
            if self.num_its_ % self.soft_comp_interval == 0:
            ...
            # Adaptive update
            if self.adaptive_ and (self.num_its_ + 1) % self.soft_comp_interval == 0:
                imp_new = self._impostors(Mnew, X, y, target_neighbors)
                err = self._compute_error(self.mu, Mnew, X, y, target_neighbors, imp_new)
                # print(err)
                if err < err_prev:
                    self.eta_ *= self.learn_inc
                else:
                    self.eta_ *= self.learn_dec
                    update = False
                    if self.eta_ < self.eta_thres:
                        stop = True
            ...
            if update:
                ...
                if imp_new is not None:
                    impostors = imp_new
                if imp_new is None and (self.num_its_ + 1) % self.soft_comp_interval == 0:
                    impostors = self._impostors(M, X, y, target_neighbors)

                tol_norm = np.max(np.abs(M - Mprev))
                grad_norm = np.max(np.abs(G))
                if tol_norm < self.tol or grad_norm < self.prec:
                    stop = True
                err_prev = err

            self.num_its_ += 1
            if self.num_its_ == self.max_iter:
                print("Max nr of iterations done")
                stop = True

        self.M_ = M
        self.final_error_ = self._compute_error(self.mu, M, X, y, target_neighbors, self._impostors(M, X, y, target_neighbors))
    ...
    def _SGD_fit(self, X, y):
        ...
        self.initial_error_ = self._compute_euc_error(self.mu, Lx, y, target_neighbors, impostors)
        err_prev = err = self.initial_error_
        stop = False
        ...
        while not stop:
            ...
            for i in rnd:
                ...
                grad = (1 - self.mu) * non_imp_grad + self.mu * imp_grad
                grad = 2 * L.dot(grad)
                L -= self.eta_ * grad
                Lx = L.dot(X.T).T
            ...
            if self.adaptive_:
                impostors = self._euc_impostors(Lx, y, target_neighbors)
                err = self._compute_euc_error(self.mu, Lx, y, target_neighbors, impostors)
                # print(err)
                if err < err_prev:
                    self.eta_ *= self.learn_inc
                else:
            ...
            if grad_norm < self.prec or self.eta_ * grad_norm < self.tol:  # Difference between two iterations is given by eta*grad
                stop = True
        ...
        self.L_ = L
        self.final_error_ = self._compute_euc_error(self.mu, Lx, y, target_neighbors, self._euc_impostors(X, y, target_neighbors))
    ...
    def predict(self, X=None):
        ...
        for t, xt in enumerate(X):
            ...
            for c in classes:
                ...
                energy = (1 - self.mu) * target_energy + self.mu * (imposter_energy + self_imposter_energy)
    ...
    def _target_neighbors(self, X, y):
        # Returns a matrix n x k, where n is the amount of data, and each row contains
        # the target neighbors indexes for each data index.

        n, d = X.shape

        unique_labels = np.unique(y)
        target_neighbors = np.empty([n, self.k], dtype=int)
        ...
        for label in unique_labels:
            ...
            target_inds = np.argsort(dists)[..., :self.k]
...
class KLMNN(KernelDML_Algorithm):
    ...
    def fit(self, X, y):
        ...
        # outers = calc_outers(X)
        if self.target_selection == "original":
            target_neighbors = self._target_neighbors(X, y)
        elif self.target_selection == "kernel":
        ...
        self.initial_error_ = self._compute_euc_error(self.mu, Lkx, y, target_neighbors, impostors)
        err_prev = err = self.initial_error_
        stop = False
        ...
        while not stop:
            ...
            if self.adaptive_:
                impostors = self._euc_impostors(Lkx, y, target_neighbors)
                err = self._compute_euc_error(self.mu, Lkx, y, target_neighbors, impostors)
        ...
        self.L_ = L
        self.final_error_ = self._compute_euc_error(self.mu, Lkx, y, target_neighbors, self._euc_impostors(X, y, target_neighbors))
